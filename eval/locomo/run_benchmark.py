"""LoCoMo benchmark on EcoDB -- session-level retrieval evaluation.

Loads 10 conversations into isolated workspaces, runs 1,986 QA queries
through GAMR search, evaluates session-level recall@K using evidence
dialog IDs as ground truth.
...
def load_conversation_sessions(client: httpx.Client, conv: dict,
                                ws_id: int, proj_id: int) -> dict:
    """Load all sessions of a conversation as memories.

    Returns mapping: session_key (e.g. "session_1") -> memory_id
    """
    c = conv["conversation"]
    session_keys = sorted(
        ...
        for attempt in range(5):
            r = client.post(f"{API_URL}/memories", headers=headers(),
                             json={
                                 ...
                                 "workspace_id": ws_id,
                                 "project_id": proj_id,
                                 "tags": [f"session:{sk}", f"date:{date}"],
                             })
            if r.status_code == 503:
                wait = 2 ** attempt
                print(f"    503 on {sk}, retry {attempt+1}/5 in {wait}s...")
                time.sleep(wait)
                continue
            r.raise_for_status()
            break
        else:
            print(f"    FAILED: {sk} after 5 retries")
            continue

        mem_id = r.json()["id"]
        mapping[sk] = mem_id
        print(f"    {sk}: {len(text)} chars -> {mem_id[:8]}")
        time.sleep(1)
...
def search_and_evaluate(client: httpx.Client, conv: dict, ws_id: int,
                         session_mapping: dict) -> list[dict]:
    """Run all QA pairs for a conversation and compute metrics."""
    memid_to_session = {v: k for k, v in session_mapping.items()}
...
def main():
    ...
    out_path = os.path.join(RESULTS_DIR, "locomo_results.jsonl")
    ...
    with httpx.Client(timeout=300) as client, open(out_path, "w") as out_f:
        for conv_idx, conv in enumerate(data):
            ...
            ws_name = f"locomo-{sample_id}"
            ws_id, proj_id = create_workspace(client, ws_name)
            print(f"  Workspace: {ws_id}, Project: {proj_id}")

            print(f"  Loading {len(session_keys)} sessions...")
            session_mapping = load_conversation_sessions(client, conv, ws_id, proj_id)
            print(f"  Loaded {len(session_mapping)}/{len(session_keys)} sessions")
            ...
            conv_results = search_and_evaluate(client, conv, ws_id, session_mapping)
    ...
    print_metrics(all_results, "EcoDB GAMR (session-level retrieval)")
