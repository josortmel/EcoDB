...
// the Bearer header inside the fetch/sse handlers. It is never exposed over the
// contextBridge. The key (and its ciphertext) is never logged.
...
function store(): Store<{ api_key_encrypted?: string }> {
  return (_store ??= new Store<{ api_key_encrypted?: string }>({ name: 'ecodb-secure' }));
}
...
export function setApiKey(raw: string): boolean {
  ...
  if (!key) return false;
  if (key.length > 512) return false; // FB2-L1 — bound the ciphertext/write size
  if (!safeStorage.isEncryptionAvailable()) {
    // Refuse to persist a plaintext key. FB3's auth screen surfaces this.
