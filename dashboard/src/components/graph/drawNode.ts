import { colorFor, hexToRgba, nodeRadius, TAU, type GNode, type NodeId } from './graphTypes';

// Multi-select ring: ice-blue, deliberately OUTSIDE the 12-type palette and ≠ the
// white focal ring / the section green (which would collide with proyecto + --grn).
const SEL_RING = '#a0c8ff';

export interface DrawOpts {
  palette: { nodeHot: string; text: string };
...
export function drawNode(node: GNode, ctx: CanvasRenderingContext2D, globalScale: number, opts: DrawOpts): void {
  const x = node.x ?? 0;
  const y = node.y ?? 0;
  const color = node.hot ? opts.palette.nodeHot : colorFor(node.type);
  const r = nodeRadius(node.degree) * opts.nodeSize;
  const gr = r * (node.hot ? 3.2 : 2.1);
  ctx.save();
  const g = ctx.createRadialGradient(x, y, 0, x, y, gr);
  g.addColorStop(0, color);
  g.addColorStop(0.45, hexToRgba(color, node.hot ? 0.42 : 0.3));
