...
// empty track = recessed neutral. The contrast is the depth Pepe asked for.
const FILL = 'color-mix(in srgb, var(--sec-graph) 82%, #000 18%)';
// Theme-adaptive recessed track (var(--inset)); the old hardcoded white-alpha was
// invisible on the light theme's bright glass (GRAPH4-M1).
  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      <input
        ...
        // accent-color paints the thumb; the gradient paints a filled dark-green
        // track up to the thumb over a recessed empty remainder (§2.5 depth).
        style={{
...
export function TunePanel({ open, onToggle, values, onChange }: TunePanelProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      ...
      {open && (
        // CARD glass (§2.5) so the Tune panel matches the right-side inspector/drawer.
        <GlassCard className="w-[190px] p-3">
          <div className="flex flex-col gap-3" data-testid="gph-tune-panel">
            <Slider label={t('gph.tune.repel')} value={values.charge} min={-400} max={-10} step={5} onChange={(v) => onChange({ charge: v })} fmt={(v) => String(Math.round(-v))} />
            <Slider label={t('gph.tune.linkDist')} value={values.linkDist} min={10} max={220} step={5} onChange={(v) => onChange({ linkDist: v })} fmt={(v) => String(Math.round(v))} />
            <Slider label={t('gph.tune.nodeSize')} value={values.nodeSize} min={0.5} max={2.5} step={0.1} onChange={(v) => onChange({ nodeSize: v })} fmt={(v) => `${v.toFixed(1)}×`} />
            <Slider label={t('gph.tune.labelZoom')} value={values.labelZoom} min={0.8} max={3} step={0.1} onChange={(v) => onChange({ labelZoom: v })} fmt={(v) => v.toFixed(1)} />
          </div>
        </GlassCard>
      )}
    </div>
