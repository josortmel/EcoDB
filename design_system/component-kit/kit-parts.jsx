...
/* ---------- A1 GlassCard ---------- */
function GlassCard({ title, tag, control, variant = "default", state = "rest", hover, msg, children, className = "", style }) {
  const pad = variant === "compact" ? { padding: "12px 14px" } : variant === "flush" ? { padding: 0 } : null;
  const cls = ["card", hover ? "is-hover" : "", state === "error" ? "is-error" : "", state === "empty" ? "is-empty" : "", className].filter(Boolean).join(" ");
  const head = (title || tag || control) && (
    <div className="card-head" style={variant === "flush" ? { padding: "14px 16px 10px" } : null}>
      <span className="title">{title}</span>{control || (tag && <span className="tag">{tag}</span>)}
    </div>
  );
  return (
    <div className={cls} style={{ ...pad, ...style }}>
      {head}
